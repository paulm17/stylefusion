...
export const plugin = createUnplugin<PigmentOptions, true>((options) => {
  ...
  const finalTransformLibraries = transformLibraries
    .concat([process.env.RUNTIME_PACKAGE_NAME as string, '@mui/material-stylefusion'])
  ...
  const babelTransformPlugin: UnpluginOptions = {
    name: 'stylefusion-plugin-transform-babel',
    enforce: 'post',
    transformInclude(id) {
  ...
  const wywInJSTransformPlugin: UnpluginOptions = {
    name: 'stylefusion-plugin-transform-wyw-in-js',
    enforce: 'post',
    buildEnd() {
      onDone(projectPath);
    },
    ...
    async transform(code, url) {
      ...
      try {
        ...
        if (isNext) {
          ...
          // Fix for https://github.com/mui/stylefusion/issues/199
  ...
  const plugins: Array<UnpluginOptions> = [
    {
      name: 'stylefusion-plugin-theme-tokens',
      enforce: 'pre',
      webpack(compiler) {
  ...
  if (!isNext) {
    plugins.push({
      name: 'stylefusion-plugin-load-output-css',
      enforce: 'pre',
      resolveId(id) {
